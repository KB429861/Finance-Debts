...
namespace Finance.Validation
{
    ...
    public class MathParser
    {
        ...
        public MathParser()
        {
            try
            {
                char.TryParse(CultureInfo.CurrentCulture.NumberFormat.NumberDecimalSeparator, out _decimalSeparator);
            }
        ...
        private string LexicalAnalysisInfixNotation(string expression, ref int pos)
        {
                ...
                || _supportedConstants.ContainsKey(token.ToString()))
            {
                ...
                if (_supportedConstants.ContainsKey(token.ToString()))
                {
                    return _supportedConstants[token.ToString()];
                }
                throw new ArgumentException("Unknown token");
            }
            if (char.IsDigit(token[0]) || token[0] == _decimalSeparator)
            {
                // Read number
